...
grad_ggc = GuidedGradCam(model, conv_module)
attr_ggc = compute_attributions(grad_ggc, X_tensor, target = y_tensor)
visualize_attr_maps('visualization/guidedgradcam_captum.png', X, y, class_names, [attr_ggc], ['GuidedGradCam'])

# Computing Guided BackProp
grad_gbp = GuidedBackprop(model)
attr_gbp = compute_attributions(grad_gbp, X_tensor, target = y_tensor)
visualize_attr_maps('visualization/guidedbackprop_captum.png', X, y, class_names, [attr_gbp], ['GuidedBackprop'])
...
attr_lgc = compute_attributions(layer_gradcam, X_tensor, target = y_tensor)
attr_lgc = torch.squeeze(attr_lgc, dim=1)
visualize_attr_maps('visualization/layer_gradcam_captum.png', X, y, class_names, [attr_lgc], ['LayerGradCam'], 
                    attr_preprocess=lambda arg:arg.detach().numpy())

# layer conductance
layer_cond = LayerConductance(model, layer)
attr_lc = compute_attributions(layer_cond, X_tensor, target = y_tensor)
attr_lc = attr_lc.mean(dim=1)
attr_lc = torch.squeeze(attr_lc, dim=1)
# print(attr_lc.shape)
visualize_attr_maps('visualization/layer_conductance.png', X, y, class_names, [attr_lc], ['LayerConductance'], 
                    attr_preprocess=lambda arg:arg.detach().numpy())
# visualize_attr_maps('visualization/layer_conductance.png', X, y, class_names, [attr_lc], ['LayerConductance'])
