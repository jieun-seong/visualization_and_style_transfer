...
# attr_ig = compute_attributions(int_grads, X_tensor, target=y_tensor, n_steps=10)
# visualize_attr_maps('visualization/int_grads_captum.png', X, y, class_names, [attr_ig], ['Integrated Gradients'])
grads = Saliency(model)
# attr = compute_attributions(grads, X_tensor, target = y_tensor)
attr = compute_attributions(grads, X_tensor, target = y_tensor)
visualize_attr_maps('visualization/saliency_captum.png', X, y, class_names, [attr], ['Saliency'])
##############################################################################
#                             END OF YOUR CODE                               #
##############################################################################
