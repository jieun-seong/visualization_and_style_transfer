...
def style_transfer(name, content_image, style_image, image_size, style_size, content_layer, content_weight,
    ...
    for t in range(200):
        ...
        """
        loss = 0.0
        loss = content_loss.forward(content_weight, feats[content_layer], content_target)
        loss += style_loss.forward(feats, style_layers, style_targets, style_weights)
        loss += tv_loss.forward(img, tv_weight)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        scheduler.step()
