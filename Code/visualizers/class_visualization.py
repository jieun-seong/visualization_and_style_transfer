from fooling_image import X_fooling
import torch
...
class ClassVisualization:
    ...
    def create_class_visualization(self, target_y, class_names, model, dtype, **kwargs):
        ...
        for t in range(num_iterations):
            ...
            ########################################################################
            model.zero_grad()
            reg = torch.norm(img_var) * l2_reg
            score = model.forward(img_var)
            score = score[0,target_y] - reg
            score.backward()
            img_var.requires_grad = False
            img_var = img_var + learning_rate * torch.nn.functional.normalize(img_var.grad)
            img = img_var.data
            img_var.requires_grad = True
            if img_var.grad != None:
                img_var.grad.zero_()
            ########################################################################
            #                             END OF YOUR CODE                         #
            ########################################################################
