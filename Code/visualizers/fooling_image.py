...
class FoolingImage:
    def make_fooling_image(self, X, target_y, model):
        ...
        for it in range(max_iter):
            ...
            ##############################################################################
            model.zero_grad()
            pred = model.forward(X_fooling_var)
            if torch.argmax(pred) != target_y:
                ...
                # print("score = ", score)
                # print("pred[target_y] = ", pred[0, target_y])
                score.backward()
                X_fooling_var.requires_grad = False
                g = X_fooling_var.grad
                dX = learning_rate * torch.nn.functional.normalize(g)
                X_fooling_var = X_fooling_var + dX
                X_fooling_var.requires_grad = True
            else:
                break
            ##############################################################################
            #                             END OF YOUR CODE                               #
