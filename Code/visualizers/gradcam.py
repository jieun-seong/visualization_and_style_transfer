...
class CustomReLU(TorchFunc):
    ...
    def backward(self, y):
        ...
        # output = torch.addcmul(torch.zeros(y.size()), y, (y > 0).type_as(y) * (x > 0).type_as(x))
        output = torch.addcmul(torch.zeros(y.size()), y, (x > 0).type_as(x))
        return output
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
...
class GradCam:
    def guided_backprop(self, X_tensor, y_tensor, gc_model):
        ...
        the input images.
        Note:
        1) Sum over all the prediction to obtain a scalar and do a single backprop over 
	    the scalar 
        ...
        s = torch.sum(pred)
        s.backward()
        output = (X_tensor > 0).type_as(X_tensor) * (X_tensor.grad > 0).type_as(X_tensor) * X_tensor.grad
    ...
    def grad_cam(self, X_tensor, y_tensor, gc_model):
        ...
        s.backward()
        relu = torch.nn.ReLU()
        cam = relu(torch.sum(self.gradient_value * self.activation_value,1)).detach().numpy()
        ##############################################################################
        #                             END OF YOUR CODE                               #
        ##############################################################################
