...
class SaliencyMap:
    def compute_saliency_maps(self, X, y, model):
        ...
        # model.zero_grad()
        loss.backward()
        # print(X_var.grad.shape)
        # with torch.no_grad():
        #     for p in model.parameters():
        #         new_param = p + p.grad * learning_rate
        saliency = torch.tensor(numpy.amax(torch.abs(X_var.grad.detach()).numpy(), axis=1))

        #### my own loss function ####
        # print(numpy.argmax(prediction.detach().numpy(), axis=1))
        # print(y_var)
